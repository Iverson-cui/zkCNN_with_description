 ...
 * This function copies the verifier challenge into prover's r_u
...
{
    sumcheck_id = C.size;
    // last_bl is the bit length of the final layer output
    i8 last_bl = C.circuit[sumcheck_id - 1].bit_length;
    // resized to accommodate last_bl random field elements
    r_u[sumcheck_id].resize(last_bl);

    prove_timer.start();
    // copying every verifier's random challenge into prover's local storage
 ...
 * in the end, it use alpha and beta to reduce multiple claims to one claim.
 */
void prover::sumcheckInit(const F &alpha_0, const F &beta_0)
{
    ...
    alpha = alpha_0;
    beta = beta_0;
    // assign two sets of random challenges to 2 iterators
...
void prover::sumcheckDotProdInitPhase1()
{
    ...
    for (u32 u = 0; u < total[1]; ++u)
    {
        ...
        if (u >= cur.size_u[1])
            ...
            V_mult[1][u] = val[sumcheck_id - 1][u];
    }
...
cubic_poly prover::sumcheckDotProdUpdate1(const F &previous_random)
{
    ...
    for (u32 i = 0; i < (total[1] >> 1); ++i)
    {
        ...
        if (total[0])
            ret = ret + tmp_mult[i & total[0] - 1] * tmp_v1[i] * tmp_v0[i];
        else
            ret = ret + tmp_mult[0] * tmp_v1[i] * tmp_v0[i];
    }
...
/**
 * This function sets up verification for the "left" inputs (u) to all gates, establishing the foundation
 */
void prover::sumcheckInitPhase1(const F &relu_rou_0)
{
    ...
    if (cur.ty == layerType::PADDING)
        beta_gs.resize(1ULL << cur.fft_bit_length);
    ...
    if (cur.ty == layerType::FFT || cur.ty == layerType::IFFT)
    {
        ...
        if (cur.ty == layerType::FFT)
            ...
            initBetaTable(beta_g, cnt_bl, r_0 + fft_blh, alpha);
    ...
    else
    {
        ...
        if (cur.ty == layerType::PADDING)
        {
            ...
            initBetaTable(beta_g, cur.bit_length, r_0, r_1, alpha * cur.scale, beta * cur.scale);
        if (cur.zero_start_id < cur.size)
            for (u32 g = cur.zero_start_id; g < 1ULL << cur.bit_length; ++g)
...
/**
 * This function Sets up verification for the "right" inputs (v) to binary gates only, completing the two-input gate verification
 * for some layers phase 2 is not needed as there are only one set of input, so this function is skipped
 */
 ...
 *
 * @param r: random field elements for evaluating MLE
 * @param output_size: how many output elements to produce
 * @param r_size: the bit length of the output
 * @return MLE of output evaluated at random points
 */
F prover::Vres(const vector<F>::const_iterator &r, u32 output_size, u8 r_size)
{
    ...
    // copy output layer values to "output" vector
    for (u32 i = 0; i < output_size; ++i)
        output[i] = val[C.size - 1][i];
    // whole represents the full size of the boolean hypercube
    // 2^(r_size)
    ...
    // after the iteration, it collapse to one finite field value, output[0]
    F res = output[0];
