...
verifier::verifier(prover *pr, const layeredCircuit &cir) : p(pr), C(cir)
{
...
F verifier::getFinalValue(const F &claim_u0, const F &claim_u1, const F &claim_v0, const F &claim_v1)
{
    ...
    auto test_value = bin_value[0] * (claim_u0 * claim_v0) + bin_value[1] * (claim_u1 * claim_v1) + bin_value[2] * (claim_u1 * claim_v0) + uni_value[0] * claim_u0 + uni_value[1] * claim_u1;
...
void verifier::predicatePhase1(u8 layer_id)
{
    ...
    if (cur_layer.ty == layerType::FFT || cur_layer.ty == layerType::IFFT)
        ...
        for (auto &gate : cur_layer.uni_gates)
        {
            bool idx = gate.lu;
...
/**
 * This function serve as the verify function.
 * call this function to do the whole circuit verification with commitment openings
 */
bool verifier::verify()
{
    ...
    // populate vector gens with random G1 elements
    for (auto &x : gens)
    {
        Fr tmp;
        tmp.setByCSPRNG();
