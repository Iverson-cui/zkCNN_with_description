...
/**
 * psize_x and psize_y is the picture width and height
 * pchannel is the number of input channels
 * i,c,o,n_filename are all files input
 * n_filename is the network configuration file, which VGG16 and VGG11 don't have
 */
vgg::vgg(i64 psize_x, i64 psize_y, i64 pchannel, i64 pparallel, const string &i_filename,
         const string &c_filename, const std::string &o_filename, const std::string &n_filename) : neuralNetwork(psize_x, psize_y, pchannel, pparallel, i_filename, c_filename, o_filename)
{
    ...
    */
    ...
    string con;
    // kernel is 3*3
    // also initialize pic_channel, pic_size_x, pic_size_y, pic_parallel
    i64 kernel_size = 3, ch_in = pic_channel, ch_out, new_nx = pic_size_x, new_ny = pic_size_y;
    // select convolution implementation: FFT for large kernels/parallel processing, otherwise NAIVE_FAST
    convType conv_ty = kernel_size > 3 || pparallel > 1 ? FFT : NAIVE_FAST;

    int idx = 0;
    // reads token from file and assign it to con
    while (config_in >> con)
    {
        // M->Maxpooling, A->Avgpooling
        // not M not A means conv layers
        if (con[0] != 'M' && con[0] != 'A')
        {
        ...
        {
            ++idx;
            // set pool parameters, including types, size and stride
...
/** vgg16 and vgg11 doesn't have n_filename
 * architecture is hard coded
 * 2+2+3+3+3, 13 conv layers; 3 FC layers, 16 layers in total
 */
...
/**
 * Architecture: 1+1+2+2+2, 8 conv layers; 3 FC layers, 11 layers in total
 */
...
/**simple CNN structure with only one conv layer
 * no FC layers
 * conv_section[1] waiting to be fixed?
 */
ccnn::ccnn(i64 psize_x, i64 psize_y, i64 pparallel, i64 pchannel, poolType pool_ty) : neuralNetwork(psize_x, psize_y, pchannel, pparallel, "", "", "")
{
...
/**
 * This function is a parameter position binding system that pre-calculates the memory layout before actual computation begins.
 * it gives each layer a pos that indicate the parameter position for later use.
 */
void singleConv::initParamConv()
{
    ...
    // data
    // data size= pic_size_x * pic_size_y * pic_channel * pic_parallel
    // after data there are weights
    i64 pos = pic_size_x * pic_size_y * pic_channel * pic_parallel;

    new_nx_in = pic_size_x;
    new_ny_in = pic_size_y;
    // iterate through each conv layer in each conv_section
    for (i64 i = 0; i < conv_section.size(); ++i)
    {
        auto &sec = conv_section[i];
        for (i64 j = 0; j < sec.size(); ++j)
        {
            ...
            // based on conv type, calculate conv layer cnt
            // conv type: FFT, NAIVE, NAIVE_FAST
            conv_layer_cnt += sec[j].ty == FFT ? FFT_SIZE - 1 : sec[j].ty == NAIVE ? NCONV_SIZE
                                                                                   : NCONV_FAST_SIZE;
            // conv_kernel
            // sec[j] kernel weight starts from pos
            sec[j].weight_start_id = pos;
            pos += sqr(m) * channel_in * channel_out;
            total_para_size += sqr(m) * channel_in * channel_out;
            // for now no bias is added
            sec[j].bias_start_id = -1;
        }
    }
    // total_in_size=input data+weights parameters
    total_in_size = pos;
